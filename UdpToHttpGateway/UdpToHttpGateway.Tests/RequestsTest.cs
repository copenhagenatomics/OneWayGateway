...
public class RequestsTest
{
 ...
 """);
    ...
    private static Task TestRequest(HttpMethod method, string relativeUri, byte[]? contentBytes, string expectedHeader) =>
        TestRequest(method, relativeUri, contentBytes != null ? new ReadOnlyMemoryContent(contentBytes) : null, expectedHeader);
    private static async Task TestRequest(HttpMethod method, string relativeUri, HttpContent? content, string expectedHeader)
    {
        Channel<(string, byte[])> received = Channel.CreateUnbounded<(string, byte[])>();
        HttpRequestMessage request = new(method, new Uri(new Uri($"http://{testIp}:{HttpPort}"), relativeUri));
        if (content != null)
            request.Content = content;
        using var gatewayClient = new GatewayClient(IPEndPoint.Parse("127.0.0.1:4280"));
        await using var web = CreateFakeServer(HttpPort, received);
        await gatewayClient.Send(request);
        await AssertFakeServerReceivedExpectedRequest(content != null ? await content.ReadAsByteArrayAsync() : [], expectedHeader, received);
        await web.StopAsync();
    }


    private static async Task AssertFakeServerReceivedExpectedRequest(byte[] expectedContent, string expectedHeader, Channel<(string, byte[])> received)
    {
        try
        {
            ...
            var (headers, content) = await received.Reader.ReadAsync(cts.Token);
    ...
    private static WebApplication CreateFakeServer(int httpPort, Channel<(string, byte[])> received)
    {
        var builder = WebApplication.CreateBuilder();
        builder.WebHost.UseShutdownTimeout(TimeSpan.FromSeconds(3));//hard stop after 3 seconds.
        var app = builder.Build();
        app.Urls.Add($"http://+:{httpPort}");
        app.Map("/", async (HttpRequest request) =>
        {
            await received.Writer.WriteAsync(await ToRaw(request));
            return "OK";
        });
        _ = app.RunAsync();
        return app;
    ...
    public async static Task<(string,byte[])> ToRaw(HttpRequest request)
    {
